...
export async function execute(interaction) {
  try {
    ...
    // ユーザーのライセンス取得
    const licensesDoc = await interaction.client.db.collection("licenses").findOne({ userId });
    const userLicenses = licensesDoc?.licenses || {};

    if (action === "add") {
      if (userLicenses[license]) {
        return await interaction.editReply(`❌ ユーザーはすでに **${license}** を所有しています`);
      }
      ...
      await interaction.editReply(`✅ <@${userId}> に **${license}** を追加しました`);
    } else if (action === "remove") {
      if (!userLicenses[license]) {
        return await interaction.editReply(`❌ ユーザーは **${license}** を持っていません`);
      }
      await interaction.client.db.collection("licenses").updateOne(
        { userId },
        { $unset: { [`licenses.${license}`]: "" } }
      );
      await interaction.editReply(`✅ <@${userId}> から **${license}** を剥奪しました`);
    } else {
      return await interaction.editReply("❌ action は add または remove を指定してください");
    }
